...
typedef enum {
    /* 云台电机 */
    CAN_DM_MOTOR_UP_PITCH_ID   = 0x01,         //2     上pitch轴
    CAN_DM_MOTOR_DOWN_PITCH_ID = 0x02,         //2     下pitch轴
    CAN_DM_MOTOR_YAW_ID        = 0x03,         //2     yaw轴
}DM_Motor_CAN_ID_e;

// DM电机的MASTER_ID
typedef enum {
    /* 云台电机 */
    MASTER_DM_MOTOR_UP_PITCH_ID   = 0x11,         //2     上pitch轴
    MASTER_DM_MOTOR_DOWN_PITCH_ID = 0x12,         //2     下pitch轴
    MASTER_DM_MOTOR_YAW_ID        = 0x13,         //2     yaw轴
...
typedef struct {
    ...
    /* 自定义数据 */
    fp32 offset_position;                   //  初始位置
    fp32 offset_angle_to_position_ratio;
    fp32 offset_position_min;
    fp32 offset_position_max;
    float kd;
}DM_Motor_t;
...
extern volatile uint8_t DM_Up_Pitch_T_Rotor;
