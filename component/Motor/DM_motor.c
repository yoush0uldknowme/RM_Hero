...
volatile uint8_t DM_Up_Pitch_T_Rotor = 0U;
...
void DM_Motor_Decode(DM_Motor_t *motor, CAN_TYPE hcan, uint32_t can_id, uint8_t *data) {
    if(hcan == CAN_2) {
        if(can_id== MASTER_DM_MOTOR_UP_PITCH_ID) {
            status_up_pitch = data[0] & 0xF0;
            if(status_up_pitch == 0) {
                DM_Send_CMD(hcan, CAN_DM_MOTOR_UP_PITCH_ID, DM_Enable_CMD);
            }
            motor->p_int = (data[1] << 8) | data[2];
            motor->v_int = (data[3] << 4) | (data[4] >> 4);
            motor->t_int = ((data[4] & 0xF) << 8) | data[5];
            DM_Up_Pitch_T_Rotor = data[7];
        ...
        else if(can_id== MASTER_DM_MOTOR_DOWN_PITCH_ID) {
            status_down_pitch = data[0] & 0xF0;
            if(status_down_pitch == 0) {
                DM_Send_CMD(hcan, CAN_DM_MOTOR_DOWN_PITCH_ID, DM_Enable_CMD);
            }
            motor->p_int = (data[1] << 8) | data[2];
            motor->v_int = (data[3] << 4) | (data[4] >> 4);
            motor->t_int = ((data[4] & 0xF) << 8) | data[5];
        ...
        else if(can_id== MASTER_DM_MOTOR_YAW_ID) {
            status_yaw = data[0] & 0xF0;
            if(status_yaw == 0) {
