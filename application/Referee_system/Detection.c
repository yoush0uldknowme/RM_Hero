...
static uint32_t up_pitch_temperature_rgb(uint8_t temperature)
{
    if(temperature < 60U)
    {
        return 0xFF00FF00;
...
void offline_remind(uint8_t offline_num,uint8_t max_level,uint8_t max_level_count)
{
    ...
    if(offline_num==0)//所有设备在线
    {
        ...
        switch (max_level) {
            ...
            case 4:{
                ...
                if(BUZZER_REMIND_ENABLE&&detect_list[DETECT_GIMBAL_UP_PITCH].status==OFFLINE)
                {
                    if(buzzer_remind_count==5||buzzer_remind_count==10)
                    {
                        buzzer_remind();
                ...
                if(BUZZER_REMIND_ENABLE&&detect_list[DETECT_GIMBAL_DOWN_PITCH].status==OFFLINE)
                {
                    if(buzzer_remind_count==5||buzzer_remind_count==10)
                    {
                        buzzer_remind();
    ...
    buzzer_remind_count++;
    rgb = up_pitch_temperature_rgb(DM_Up_Pitch_T_Rotor);
    //点灯
    aRGB_led_show(rgb);
}
...
void Detect_task(void const*pvParameters){
    ...
    detect_init(DETECT_GIMBAL_UP_PITCH,500,4);
    detect_init(DETECT_GIMBAL_DOWN_PITCH,500,4);
