...
void Chassis_Steering_Wheel_Cal(void) {
    ...
    const fp32 half_wheelbase = WHEELBASE * 0.5f;
    const fp32 half_wheeltrack = WHEELTRACK * 0.5f;

    Vx[RF] = chassis.vx - chassis.vw * half_wheeltrack;
    Vy[RF] = chassis.vy + chassis.vw * half_wheelbase;

    Vx[LF] = chassis.vx + chassis.vw * half_wheeltrack;
    Vy[LF] = chassis.vy + chassis.vw * half_wheelbase;

    Vx[LB] = chassis.vx + chassis.vw * half_wheeltrack;
    Vy[LB] = chassis.vy - chassis.vw * half_wheelbase;

    Vx[RB] = chassis.vx - chassis.vw * half_wheeltrack;
    Vy[RB] = chassis.vy - chassis.vw * half_wheelbase;
    ...
    for (uint8_t i = 0; i < 4; i++) {
        ...
        if (speed < 1e-3f) {
            ...
            // 正常计算目标角度
            fp32 angle = atan2f(Vy[i], Vx[i]);
            fp32 current_angle = chassis.Steering_motor_chassis[i].relative_angle_get * DEG_TO_RAD;

            fp32 angle_diff = angle - current_angle;
            fp32 reverse_enter = PI / 2.0f + STEERING_REVERSE_HYSTERESIS_DEG * DEG_TO_RAD;
            fp32 reverse_exit = PI / 2.0f - STEERING_REVERSE_HYSTERESIS_DEG * DEG_TO_RAD;
            fp32 reverse_direction = chassis.drive_motor_chassis[i].Direction;
            while (angle_diff > PI) angle_diff -= 2.0f * PI;
            while (angle_diff < -PI) angle_diff += 2.0f * PI;

            if (reverse_direction == 0.0f) {
                reverse_direction = 1.0f;
            }

            if (fabsf(angle_diff) > reverse_enter) {
                reverse_direction = -1.0f;
            } else if (fabsf(angle_diff) < reverse_exit) {
                reverse_direction = 1.0f;
            }
            ...
            chassis.drive_motor_chassis[i].Direction = reverse_direction;
    ...
    predicted_power = steering_predicted_power + drive_predicted_power;
        ...
        else if(power_boost_on == 1)
        {
            ...
            if(cap_is_on == 0) {
                ...
                // 如果电容在线，闭环电容能量
                drive_power_budget += cap_Get_Boosting_Power(25.0f, 10.0f, 7.0f);
            }
...
int16_t drive_given_current_RF;
int16_t drive_given_current_LF;
int16_t drive_given_current_LB;
int16_t drive_given_current_RB;

int16_t drive_rpm_RF;
int16_t drive_rpm_LF;
int16_t drive_rpm_LB;
int16_t drive_rpm_RB;
...
void Chassis_Wheel_Loop_Cal(void) {
    drive_rpm_RF = chassis.drive_motor_chassis[0].motor_measure.speed_rpm;
    drive_rpm_LF = chassis.drive_motor_chassis[1].motor_measure.speed_rpm;
    drive_rpm_LB = chassis.drive_motor_chassis[2].motor_measure.speed_rpm;
    drive_rpm_RB = chassis.drive_motor_chassis[3].motor_measure.speed_rpm;

    drive_given_current_RF = chassis.drive_motor_chassis[0].motor_measure.given_current;
    drive_given_current_LF = chassis.drive_motor_chassis[1].motor_measure.given_current;
    drive_given_current_LB = chassis.drive_motor_chassis[2].motor_measure.given_current;
    drive_given_current_RB = chassis.drive_motor_chassis[3].motor_measure.given_current;
    ...
    for (int i = 0; i < 4; i++) {
        before_current[i] = chassis.Steering_motor_chassis[i].give_current;
    }
