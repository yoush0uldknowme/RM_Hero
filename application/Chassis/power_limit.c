...
void calc_power_limit(pid_type_def *pid,int i)
{
    ...
    chassis.chassis_power_limit.M[i]=-(pid->p*chassis.drive_motor_chassis[i].motor_measure.speed_rpm-pid->iout)*CHASSIS_CURRENT_CONVERT;
...
//    cap_info_update();
    for (int i = 0; i < 4; ++i) {
        //计算K,M
        calc_power_limit(&chassis.drive_motor_chassis[i].speed_p,i);
    }
    float a=0,b=0,c=0;
    for (int i = 0; i < 4; ++i) {
        //a,b,c的计算过程，具体参看华中科技大学功率控制模型
        a += CHASSIS_POWER_R0 * pow(chassis.drive_motor_chassis[i].rpm_set,2) * pow(chassis.chassis_power_limit.K[i],2);
        b += chassis.drive_motor_chassis[i].rpm_set * (2 * CHASSIS_POWER_R0 * chassis.chassis_power_limit.K[i] * chassis.chassis_power_limit.M[i] + CHASSIS_POWER_K0 * chassis.chassis_power_limit.K[i] * chassis.drive_motor_chassis[i].motor_measure.speed_rpm);
        c += CHASSIS_POWER_R0 * pow(chassis.chassis_power_limit.M[i],2) + CHASSIS_POWER_K0 * chassis.chassis_power_limit.M[i] * chassis.drive_motor_chassis[i].motor_measure.speed_rpm;
...
//        }
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            chassis.drive_motor_chassis[i].rpm_set *= chassis.chassis_power_limit.k_c;
        }
