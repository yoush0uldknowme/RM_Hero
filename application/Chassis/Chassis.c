...
static uint8_t Chassis_Referee_Shoot_Seq_Update(void);
    ...
    while (1){
        ...
        if (chassis.mode != CHASSIS_RELAX) {
            ...
            DJI_Send_Motor_Mapping(CAN_1,
                                   ...
                                   chassis.drive_motor_chassis[RF].give_current,   //207
                                   chassis.drive_motor_chassis[LF].give_current    //208
            );
            ...
            //                        0,   //201
            //                        0,   //202
            //                        chassis.drive_motor_chassis[RF].give_current,   //203
            //                        chassis.drive_motor_chassis[LF].give_current   //204
            // );
            // DJI_Send_Motor_Mapping(CAN_1,
            //                        CAN_DJI_MOTOR_0x1FF_ID,
            //                        0,   //205
            //                        0,   //206
            //                        chassis.Steering_motor_chassis[RF].give_current,   //207
            //                        chassis.Steering_motor_chassis[LF].give_current    //208
            // );
            //
            //
            // /* 后侧轮组 */
            // DJI_Send_Motor_Mapping(CAN_2,
            //                        CAN_DJI_MOTOR_0x200_ID,
            //                        chassis.drive_motor_chassis[LB].give_current,   //201
            //                        chassis.drive_motor_chassis[RB].give_current,   //202
            //                        0,   //203
            //                        0    //204
            // );
            // DJI_Send_Motor_Mapping(CAN_2,
            //                        CAN_DJI_MOTOR_0x1FF_ID,
            //                        chassis.Steering_motor_chassis[LB].give_current,   //205
            //                        chassis.Steering_motor_chassis[RB].give_current,   //206
            //                        0,   //207
            //                        0    //208
            // );
            ...
            /* 底盘各个方向的速度 */
            Chassis_Test_Data();
            roll_chassis = INS_angle[1] * MOTOR_RAD_TO_ANGLE;
        }
        roll_chassis = INS_angle[1] * MOTOR_RAD_TO_ANGLE;
...
static void Chassis_Pc_Ctrl(void) {
    ...
    fp32 vx_pc_limit = MAX_CHASSIS_AUTO_VX_SPEED *
                       (chassis.shift_slow_mode ? CHASSIS_SHIFT_SLOW_FACTOR : 1.0f);
    fp32 vy_pc_limit = MAX_CHASSIS_AUTO_VY_SPEED *
                       (chassis.shift_slow_mode ? CHASSIS_SHIFT_SLOW_FACTOR : 1.0f);
    /* 优先处理组合键 */
...
static void Chassis_Ctrl_Info_Get(void) {
    ...
    chassis.vw_set=(float)(rc_ctrl.rc.ch[CHASSIS_Z_CHANNEL])*RC_TO_VW
                   +chassis.vw_pc;
    if (gimbal.mode == GIMBAL_UP_DISABLE ||
        gimbal.mode == GIMBAL_DOWN) {
        VAL_LIMIT(chassis.vw_set,
                  -CHASSIS_PRONE_DIRECT_VW_MAX,
                   CHASSIS_PRONE_DIRECT_VW_MAX);
    }

    //TODO:怕翻车时打开
    /*斜坡，防止翻车*/
    // ramp_speed_calc(&chassis_vx_ramp,(float)(rc_ctrl.rc.ch[CHASSIS_X_CHANNEL])*RC_TO_VX+chassis.vx_pc);
    // chassis.vx_set = chassis_vx_ramp.out;
    // ramp_speed_calc(&chassis_vy_ramp,(float) (rc_ctrl.rc.ch[CHASSIS_Y_CHANNEL])*RC_TO_VY+chassis.vy_pc);
    // chassis.vy_set = chassis_vy_ramp.out;
    //
...
static void Send_Chassis_Data(void) {
    ...
    Send_referee(feedback_frame.voltage,
                 Referee.GameRobotStat.chassis_power_limit,
                 Referee.GameRobotStat.robot_id,
                 Referee.GameRobotStat.shooter_barrel_heat_limit);
    Send_bullet_speed(Referee.ShootData.bullet_speed,
                      Referee.PowerHeatData.shooter_42mm_barrel_heat,
                      Referee.GameState.game_progress,
                      Chassis_Referee_Shoot_Seq_Update(),
                      Referee.BulletRemaining.projectile_allowance_42mm);
    vTaskDelay(CHASSIS_PERIOD);
}

static uint8_t Chassis_Referee_Shoot_Seq_Update(void)
{
    static uint16_t last_shooter_heat = 0U;
    static uint8_t shoot_seq = 0U;
    uint16_t shooter_heat = Referee.PowerHeatData.shooter_42mm_barrel_heat;

    if (shooter_heat > last_shooter_heat + 5U) {
        shoot_seq++;
    }

    last_shooter_heat = shooter_heat;
    return shoot_seq;
}
...
fp32 chassis_follow_fb_debug = 0.0f;
fp32 chassis_follow_gyro_debug = 0.0f;
fp32 chassis_follow_damp_debug = 0.0f;
fp32 chassis_prone_vw_debug = 0.0f;
static void Chassis_Test_Data(void) {
    // //计算底盘在方向上的线速度
...
static void Chassis_Follow_Gimbal_Handle(void){
    ...
    if (gimbal.mode == GIMBAL_UP_DISABLE ||
        ...
        fp32 prone_vxy = sqrtf(chassis.vx * chassis.vx + chassis.vy * chassis.vy);
        if (prone_vxy > CHASSIS_PRONE_DIRECT_VXY_MAX) {
            fp32 prone_vxy_scale = CHASSIS_PRONE_DIRECT_VXY_MAX / prone_vxy;
            chassis.vx *= prone_vxy_scale;
            chassis.vy *= prone_vxy_scale;
        }

        chassis_prone_vw_debug = chassis.vw;
        chassis_follow_fb_debug = 0.0f;
        chassis_follow_gyro_debug = 0.0f;
        chassis_follow_damp_debug = 0.0f;

        VAL_LIMIT(chassis.vw,
                  -CHASSIS_PRONE_DIRECT_VW_MAX,
                   CHASSIS_PRONE_DIRECT_VW_MAX);
        VAL_LIMIT(chassis.vx, -vx_limit * (1.0f - fabsf(chassis.vw / 4.4f)),
                  vx_limit * (1.0f - fabsf(chassis.vw / 4.4f)));
        VAL_LIMIT(chassis.vy, -vy_limit * (1.0f - fabsf(chassis.vw / 4.4f)),
                  vy_limit * (1.0f - fabsf(chassis.vw / 4.4f)));
        return;
    }
    ...
    uint8_t follow_down = (gimbal.mode == GIMBAL_DOWN ||
                           gimbal.mode == GIMBAL_UP_DISABLE);
    fp32 follow_kp = follow_down ? CHASSIS_FOLLOW_DOWN_PID_KP : CHASSIS_FOLLOW_GIMBAL_PID_KP;
    fp32 follow_kd = follow_down ? CHASSIS_FOLLOW_DOWN_PID_KD : CHASSIS_FOLLOW_GIMBAL_PID_KD;
    fp32 follow_max_out = follow_down ? CHASSIS_FOLLOW_DOWN_PID_MAX_OUT : CHASSIS_FOLLOW_GIMBAL_PID_MAX_OUT;
    fp32 follow_gyro_damp = follow_down ? CHASSIS_FOLLOW_DOWN_GYRO_DAMP : CHASSIS_FOLLOW_GIMBAL_GYRO_DAMP;
    fp32 follow_linear_err = follow_down ? CHASSIS_FOLLOW_DOWN_LINEAR_ERR : CHASSIS_FOLLOW_GIMBAL_LINEAR_ERR;
    fp32 follow_large_scale = follow_down ? CHASSIS_FOLLOW_DOWN_LARGE_SCALE : CHASSIS_FOLLOW_GIMBAL_LARGE_SCALE;
    fp32 follow_small_err = follow_down ? CHASSIS_FOLLOW_DOWN_SMALL_ERR : CHASSIS_FOLLOW_GIMBAL_SMALL_ERR;
    fp32 follow_small_max_out = follow_down ? CHASSIS_FOLLOW_DOWN_SMALL_MAX_OUT : CHASSIS_FOLLOW_GIMBAL_SMALL_MAX_OUT;
    fp32 follow_full_err = follow_down ? CHASSIS_FOLLOW_DOWN_FULL_ERR : CHASSIS_FOLLOW_GIMBAL_FULL_ERR;
    fp32 follow_yaw_kff = follow_down ? CHASSIS_FOLLOW_DOWN_YAW_KFF : CHASSIS_FOLLOW_GIMBAL_YAW_KFF;
    fp32 follow_ff_max = follow_down ? CHASSIS_FOLLOW_DOWN_FF_MAX : CHASSIS_FOLLOW_GIMBAL_FF_MAX;

    chassis.chassis_vw_pid.p = follow_kp;
    chassis.chassis_vw_pid.d = follow_kd;
    chassis.chassis_vw_pid.max_output = follow_max_out;

    fp32 yaw_pid_get = chassis.board_yaw_angle;
    fp32 yaw_abs_err = fabsf(yaw_pid_get);
    if (follow_down) {
        fp32 down_vxy_max = (yaw_abs_err > 0.45f) ? CHASSIS_FOLLOW_DOWN_VXY_SAFE_MAX :
                                                    CHASSIS_FOLLOW_DOWN_VXY_MAX;
        fp32 down_vxy = sqrtf(chassis.vx * chassis.vx + chassis.vy * chassis.vy);
        if (down_vxy > down_vxy_max) {
            fp32 down_vxy_scale = down_vxy_max / down_vxy;
            chassis.vx *= down_vxy_scale;
            chassis.vy *= down_vxy_scale;
        }
    }
    if (yaw_abs_err > follow_linear_err) {
        fp32 yaw_sign = (yaw_pid_get > 0.0f) ? 1.0f : -1.0f;
        yaw_pid_get = yaw_sign * (follow_linear_err +
                      (yaw_abs_err - follow_linear_err) *
                      follow_large_scale);
    }

    fp32 follow_vw_fb = -pid_calc(&chassis.chassis_vw_pid, yaw_pid_get, 0);
    fp32 chassis_yaw_gyro = INS_gyro[INS_GYRO_Z_ADDRESS_OFFSET];
    fp32 follow_vw_damp = chassis_yaw_gyro * follow_gyro_damp;
    if (follow_down && yaw_abs_err > CHASSIS_FOLLOW_DOWN_SAFE_ERR &&
        follow_vw_fb * follow_vw_damp < 0.0f) {
        follow_vw_damp = 0.0f;
    }
    fp32 follow_vw_ff = chassis.board_yaw_gyro * follow_yaw_kff;
    VAL_LIMIT(follow_vw_ff, -follow_ff_max, follow_ff_max);
    chassis_follow_fb_debug = follow_vw_fb;
    chassis_follow_gyro_debug = chassis_yaw_gyro;
    chassis_follow_damp_debug = follow_vw_damp;
    chassis.vw = follow_vw_fb + follow_vw_ff + follow_vw_damp;

    // if(fabsf(chassis.vw) < 0.1f){
    // chassis.vw=0;
    // }
    // 确保变量val的值在指定的范围内，即不小于最小值min且不大于最大值max
    // 大角度允许更强跟随，接近零位时收小输出，减少过零反打导致的回摆。
    fp32 follow_vw_limit = follow_max_out;
    if (yaw_abs_err < follow_full_err) {
        fp32 limit_ratio = (yaw_abs_err - follow_small_err) /
                           (follow_full_err - follow_small_err);
        VAL_LIMIT(limit_ratio, 0.0f, 1.0f);
        follow_vw_limit = follow_small_max_out +
                          (follow_max_out - follow_small_max_out) *
                          limit_ratio;
    }
    VAL_LIMIT(chassis.vw, -follow_vw_limit, follow_vw_limit);
    {
        fp32 chassis_speed_factor = chassis.shift_slow_mode ? CHASSIS_SHIFT_SLOW_FACTOR : 1.0f;
        fp32 vx_limit = MAX_CHASSIS_VX_SPEED * chassis_speed_factor;
        fp32 vy_limit = MAX_CHASSIS_VY_SPEED * chassis_speed_factor;
...
void Gimbal_to_Chassis_Can(uint32_t can_id, const uint8_t *rx_data) {
    switch (can_id) {
        ...
        case 0x111:{
            union ctrl_ch angle1;
            union ctrl_ch gyro1;
            union ctrl_ch prone_vw1;
            {
                KeyBoard.W.status = (rx_data[0] & (1U << 0)) ? KEY_PRESS : KEY_RELAX;
                KeyBoard.A.status = (rx_data[0] & (1U << 1)) ? KEY_PRESS : KEY_RELAX;
                KeyBoard.S.status = (rx_data[0] & (1U << 2)) ? KEY_PRESS : KEY_RELAX;
                KeyBoard.D.status = (rx_data[0] & (1U << 3)) ? KEY_PRESS : KEY_RELAX;
                angle1.data[0] = rx_data[1];
                angle1.data[1] = rx_data[2];
                gyro1.data[0] = rx_data[3];
                gyro1.data[1] = rx_data[4];
                prone_vw1.data[0] = rx_data[5];
                prone_vw1.data[1] = rx_data[6];
            }
            fp32 yaw_angle_now = (fp32)angle1.value * 0.01f;
            chassis.vw_pc = (fp32)prone_vw1.value * 0.001f;
            static fp32 yaw_gyro_filter = 0.0f;
            static uint32_t yaw_tick_last = 0U;
            uint32_t yaw_tick_now = HAL_GetTick();
            uint32_t yaw_dt_ms = yaw_tick_now - yaw_tick_last;

            if (yaw_tick_last != 0U && yaw_dt_ms < 50U) {
                fp32 yaw_gyro_raw = (fp32)gyro1.value * 0.1f;
                VAL_LIMIT(yaw_gyro_raw,
                          -CHASSIS_FOLLOW_GIMBAL_YAW_GYRO_MAX,
                           CHASSIS_FOLLOW_GIMBAL_YAW_GYRO_MAX);
                yaw_gyro_filter += (yaw_gyro_raw - yaw_gyro_filter) *
                                   CHASSIS_FOLLOW_GIMBAL_YAW_GYRO_LPF;
            } else {
                yaw_gyro_filter = 0.0f;
            }

            chassis.board_yaw_angle = yaw_angle_now;
            chassis.board_yaw_gyro = yaw_gyro_filter;
            yaw_tick_last = yaw_tick_now;
        } break;
        ...
        case 0x112: {
            {
                ...
                vtm_rx_data.trigger = rx_data[4];
                robot_ctrl.fire_command = (rx_data[5] != 0) ? 1 : 0;
            }
        } break;
        ...
        case 0x117: {
            ...
            gimbal.up_pitch.relative_up_down_get = angle1.mode;
        }break;

        case 0x116: {
            union referee distance_data;
            distance_data.data[0] = rx_data[1];
            distance_data.data[1] = rx_data[2];
            distance_data.data[2] = rx_data[3];
            distance_data.data[3] = rx_data[4];
            chassis.board_up_pitch_temp = rx_data[0];
            chassis.board_distance = distance_data.referee;
        } break;
