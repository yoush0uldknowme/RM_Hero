...
#define TRIGGER_ANGLE_PID_KP        0.50f//0.45f//0.65f//0.2f//0.2f//0.65f//0.7f//0.2f//0.7f
#define TRIGGER_ANGLE_PID_KI        0.f//0.05f//0.f//0.05f
#define TRIGGER_ANGLE_PID_KD        3.f//7.f//5.f//2.f//1.0f//0.f//1.5f
#define TRIGGER_ANGLE_PID_MAX_IOUT  0
#define TRIGGER_ANGLE_PID_MAX_OUT   3000//2800//2600//3000

#define TRIGGER_SPEED_PID_KP        7.2f//6.2f//5.2f//4.5f//5.5f//3.f//3.f//5.f//8.f//4.f//8.f
#define TRIGGER_SPEED_PID_KI        0.f//0.005f
#define TRIGGER_SPEED_PID_KD        5.f//6.f//7.f//12.f//10.f//5.f//10.f
#define TRIGGER_SPEED_PID_MAX_IOUT  0
#define TRIGGER_SPEED_PID_MAX_OUT   13000//28000//25000//22000//18000//20000//25000
...
void Launcher_Control(void) {
    if(gimbal.mode == GIMBAL_RELAX) Launcher_Relax_Handle();
        ...
        if (launcher.fire_mode == Fire_ON) {
            launcher.fire_r.speed = -(FIRE_SPEED_R - fire_speed);
            launcher.fire_l.speed = FIRE_SPEED_L- fire_speed;
            //动态调整摩擦轮转速
            ...
            //弹速12
            if(bullet_speed != last_bullet_speed) {
                if(bullet_speed > 11.8) {
                    fire_speed +=30;
                }
                if(bullet_speed < 11.4  ) {
                    fire_speed -=30;
                }
                if(bullet_speed > 12) {
                    fire_speed +=50;
            ...
            //         fire_speed +=50;
            //     }
            //     if (bullet_speed >= 11.3 && bullet_speed <= 11.7)
            //     {
            //         if (bullet_speed > last_bullet_speed)
            //         {
            //             fire_speed += 3;
            //         }
            //         else if (bullet_speed < last_bullet_speed)
            //         {
            //             fire_speed -= 3;
            //         }
            //     }
