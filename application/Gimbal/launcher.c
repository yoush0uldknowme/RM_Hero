...
void Launcher_Init(void) {
    ...
    VSP_PID_Init(&launcher.fire_on.feedforward_speed_p,
...
void Launcher_Control(void) {
    if(gimbal.mode == GIMBAL_RELAX) Launcher_Relax_Handle();
        ...
        if (launcher.fire_mode == Fire_ON) {
            ...
            launcher.fire_on.speed = -(FIRE_SPEED_ON- fire_speed);
            //动态调整摩擦轮转速
        ...
        state_on = update_system(launcher.fire_on.speed, launcher.fire_on.motor_measure.speed_rpm, dt, state_on);
        launcher.fire_on.give_current = -smc_controller(state_on, params);
